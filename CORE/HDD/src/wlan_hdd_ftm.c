...
//    tPttMsgbuffer *pMsgBuf;
    ...
    if(!(channel >= 1 && channel <= 14) && !(channel >=36 && channel <= 252))
    {
...
static VOS_STATUS wlan_ftm_priv_set_pwr_cntl_mode(hdd_adapter_t *pAdapter,v_U16_t pwr_mode)
{
//    tPttMsgbuffer *pMsgBuf;
    uPttMsgs *pMsgBody;
    VOS_STATUS status;
    ...
    if(pwr_mode > 2 )
    {
         VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL, "%s:invalid control mode. valid mode is 0 , 1, 2. ",__func__);
         return VOS_STATUS_E_FAILURE;
    }
...
//    tPttMsgbuffer *pMsgBuf;
    ...
    pMsgBody->CloseTpcLoop.tpcClose = 0;
    ...
    init_completion(&pHddCtx->ftm.ftm_comp_var);
    pMsgBody->CloseTpcLoop.tpcClose = 2;
    status = wlan_ftm_postmsg((v_U8_t*)pMsgBuf,pMsgBuf->msgBodyLength);
...
static VOS_STATUS wlan_ftm_priv_enable_dpd(hdd_adapter_t *pAdapter,v_U16_t enable)
{
    tANI_U32 value = 0x3c ;
    hdd_context_t *pHddCtx = (hdd_context_t *)pAdapter->pHddCtx;
    if(pHddCtx->ftm.ftm_state != WLAN_FTM_STARTED)
    ...
    wpalWriteRegister(0x03018000, value);
    return VOS_STATUS_SUCCESS;
}
                       ...
                       union iwreq_data *wrqu, char *extra)
{
    ...
    switch(sub_cmd)
    {
        ...
        case WE_SET_PWR_CNTL_MODE:
        {
             ...
             if(status != VOS_STATUS_SUCCESS)
             {
                  hddLog(VOS_TRACE_LEVEL_FATAL,"wlan_ftm_priv_set_pwr_cntl_mode Failed =%d\n",status);
                  ret = -EINVAL;
              }
              break;
        }
        ...
        case WE_ENABLE_DPD:
        {
            status  = wlan_ftm_priv_enable_dpd(pAdapter,set_value);
             if(status != VOS_STATUS_SUCCESS)
             {
                  hddLog(VOS_TRACE_LEVEL_FATAL,"wlan_ftm_priv_enable_dpd Failed =%d\n",status);
                  ret = -EINVAL;
             }
             break;
        }
